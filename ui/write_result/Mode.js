const op = {
    ...
    ifRes(type, arg1, arg2) {
        switch (type) {
            case 'p***':
                return arg1 < 0.001 ? 'p < 0.001' : 'p = ' + arg1.toFixed(3);
            case 'p*':
                return arg1 < 0.05 ? '显著' : '不显著';
            case 'M':
                return arg1.M > arg2.M ? '大于' : '小于';
            case 'ia':
                return arg1 + (arg1.includes('×') ? '交互作用' : '主效应');
            case 'orient':
                return arg1 > 0 ? '正' : '反';
            case '?':
                return arg2[+arg1];
...
class Mode {
                ...
                addElm(
                    'div',
                    {
                        ...
                        onkeypress(e) {
                            switch (e.code) {
                                case 'Enter':
                                    this.$('.editor input[type=button]').click();
                                    break;
                                default:
                                    break;
                            }
                    ...
                    switch (e.type) {
                        case 'number':
                            switch (e.placeholder) {
                                case 'p':
                                    return (Math.random() * 0.1).toFixed(3);
                                case 'df':
                            ...
                            switch (e.placeholder) {
                                ...
                                default:
                                    return `${String.fromCharCode(
                                        ...[0, 0].map((e) =>
                                            parseInt(Math.random() * (90 - 65 + 1) + 65, 10),
                                        ),
                                    )}`;
                            }
                        default:
    ...
    static update() {
        ...
        vArr.filter((v) => v.elm ?? console.warn('找不到元素', v)).forEach((v) => {
            window[`${v.type}s`] = [];
...
new Mode('差异检验').config = {
    boxes: [
        {
            ...
            output() {
                ...
                const indepInfo = is.map(
                    ({ name, ls }) => `${ls.length}(${name}: ${ls.join(' / ')})`,
                );
                return `对${ds.map((e) => e.name).join('、')}采用${
                    is.length > 1 ? indepInfo.join('×') : ''
                }${method}。结果发现，`;
            },
            initFunc() {
                $('#dep').addTools(1);
        ...
        },
        {
            ...
            datas() {
                return [...this.$('.data').$('tbody').children];
            },
            confirm() {
            ...
            update() {
                ...
                const head =
                    ...
                    i1.ls.map((e, i) => `<th id="i1l${i + 1}">${e}</th>`).join('') +
                    '</tr>';
                const body = i2.ls
                    .map(
                        (e, i) =>
            ...
            output() {
                return (function ({ ls, t, df, p, CohenD, ciL, ciU }) {
                    ...
                    const msd1 = op.MSD(l1),
                        msd2 = op.MSD(l2);
                    return `${op.sign(p, l1, l2, msd1, msd2)}, t(${df}) = ${t.toFixed(
                        2,
                    )}, ${op.ifRes('p***', p)}, Cohen d = ${CohenD.toFixed(
                        2,
                    )}, 90%CI[${`${ciL.toFixed(2)}, ${ciU.toFixed(2)}`}]。`;
                })(i1);
            },
        },
        {
                ...
                return is
                    .flatMap((e, i) =>
                        is.choose(i + 1).map((es) => {
                            ...
                            return this.dataElm(
                                `${id}_Ftest`,
                                'F,df1,df2,p,eta2|η2,ciL|CI下限,ciU|CI上限',
                                {
                                    elms: [
                                        {
            ...
            output() {
                return (
                    this.is
                        .map(
                            ({ name, F, df1, df2, p, eta2, ciL, ciU }) =>
                                `${op.ifRes('ia', name)}${op.ifRes(
                                    'p*',
                                    p,
                                )}, F(${df1}, ${df2}) = ${F.toFixed(2)}, ${op.ifRes(
                                    'p***',
                                    p,
                                )}, η2 = ${eta2.toFixed(2)}, 90%CI[${`${ciL.toFixed(
                                    2,
                                )}, ${ciU.toFixed(2)}`}]`,
                        )
                        .join('；') + '。'
    ...
    post(id) {
        return {
            ...
            update() {
                return this.i.ls
                    ...
                    .map((e, i) =>
                        this.dataElm(
                            `${id}_post${i}`,
                            `l1|水平1|text|value="${e[0]}",l2|水平2|text|value="${e[1]}",p`,
                            { hasDel: 1 },
                        ),
                    )
                    .join('');
            ...
            output() {
                return `对${this.i.name}进行事后检验发现，${this.i.post
                    .map(
                        ({ l1, l2, p }) =>
    ...
    simple(id) {
        return {
            ...
            innerHTML: `<label>${window[id].name}</label>
                    <div class="data"></div>`,
            confirm() {
                this.writeData(this.$(`.data`), 'simple');
            },
            update() {
            ...
            output() {
                return `${this.i.name}的简单效应分析发现，${this.i.simple
                    .map(({ l1, l2, p, fi, fil }) => {
                        const msd1 = op.MSD(fil, l1),
                            msd2 = op.MSD(fil, l2);
                        return `${fi}为${fil}时，${op.sign(p, l1, l2, msd1, msd2)}, ${op.ifRes(
                            'p***',
                            p,
                        )}`;
                    })
...
new Mode('线性回归').boxes = [
    {
        ...
        output() {
            return `以${ds[0].name}为因变量，${is.map((e) => e.name).join('、')}为自变量，进行标准${
                is.length > 1 ? '多元' : ''
            }线性回归，各变量的相关矩阵见表1。`;
        },
        initFunc() {
    ...
    },
    {
        id: 'LinearModel',
        confirm() {
            this.writeData(this.$('#model_data'));
        },
        update() {
            ...
            return html.replace(
                /df2\"/,
                `$& onchange="$('#Linear .data div[id] input:nth-child(4)',1).forEach(e=>e.value=this.value)"`,
            );
        },
        output() {
            const { R2, F, df1, df2, p } = window['model'];
            return `回归分析结果显示，整体模型调整后的解释方差为${(R2 * 1e2).toFixed(
                1,
            )}%, F(${df1}, ${df2}) = ${F.toFixed(2)}, ${op.ifRes('p***', p)}。`;
        },
        initFunc() {
            this.$('textarea').style.display = 'none';
        },
    },
    {
        ...
        update() {
            return is
                .map(({ id }) =>
                    this.dataElm(`${id}_Linear`, 'beta|标准β,t,df,p', {
                        elms: [
                            {
                                elm: `<label>${window[id].name}</label>`,
                            },
        ...
        output() {
            return (
                ...
                this.is
                    .map(
                        ({ name, beta, t, df, p, ls }) =>
                            `${name}的${op.ifRes('orient', beta)}向预测作用${op.ifRes(
                                'p*',
                                p,
                            )}, β = ${beta.toFixed(2)}, t(${df}) = ${t}, ${op.ifRes('p***', p)}`,
                    )
                    .join('；') +
                '。'
            );
...
new Mode('中介模型').boxes = [
    {
        ...
        output() {
            return `通过回归分析考察在${i1.name}对${d1.name}的影响中${ms
                .map((e) => e.name)
                .join('、')}的中介作用，结果如图1所示。`;
        },
        initFunc() {
    ...
    },
    {
        id: 'Mediation',
        datas() {
            return this.$('.data>div[id]', 1);
        },
        confirm() {
            this.datas().forEach((e) => this.writeData(e, 'args'));
        },
        update() {
            this.models = [];
            ...
            return this.models
                .map(
                    ({ y, x }) =>
                        `<div id="${y.id}_model">${x
                            .map((e, i) =>
                                this.dataElm(
                                    e.id,
                                    `name|预测变量|text|value="${e.name}",beta|标准β,p`,
                                    {
                                        hasDel: 1,
                                        label: i ? ' ' : y.name,
                                    },
                    ...
                    .map(
                        ({ y }) =>
                            ...
                            y.args
                                .map(
                                    ({ name, beta, p }) =>
                                        `${name}对${y.name}的${op.ifRes(
                                            'orient',
                                            beta,
                                        )}向预测作用${op.ifRes('p*', p)}，β = ${beta.toFixed(
                                            2,
                                        )}, ${op.ifRes('p***', p)}`,
                                )
                                .join('，'),
    ...
    },
    {
        id: 'Indirect',
        confirm() {
            this.paths = this.datas().map(this.writeData);
        },
        update() {
            return ms
                .map((e, i, arr) =>
                    arr
                        ...
                        .map((es) => {
                            ...
                            return this.dataElm(
                                `${path}`,
                                'effect|间接效应,rate|占比,ciL|CI下限,ciU|CI上限',
                                {
                                    hasDel: 1,
                                    label: path,
                                },
        ...
        output() {
            return (
                this.paths
                    .map(
                        ({ name, effect, rate, ciL, ciU }) =>
                            `${name}：间接效应为${effect.toFixed(2)}，占总效应${(
                                rate * 1e2
                            ).toFixed(2)}%，95%BootCI[${ciL.toFixed(2)}, ${ciU.toFixed(2)}]`,
                    )
                    .join('；') + '。'
            );
        },
...
new Mode('调节模型').boxes = [
    ...
    },
    {
        id: 'Moderation',
        confirm() {
            $('#Moderation ~ .box', 1).forEach((e) => e.remove());
        },
        update() {
            return this.dataElm('demo', 'a,b,c,d', { label: '示例' });
        },
        output() {
            return ``;
        },
    },
    {
        ...
        update() {
            return (
                `选点法` +
                ['-1', '0', '+1']
                    .map((e) => this.dataElm(`m1_simple1`, 'B,t,df,p', { label: e }))
                    .join('')
                    .replace(
                        /df\"/,
                        `$& onchange="$('#m1_simple div[id] input[placeholder=df]',1).forEach(e=>e.value=this.value)"`,
                    )
            );
        },
        output() {
            return (
                ...
                m1.simple
                    .map(
                        ({ B, t, df, p }, i) =>
                            `当${m1.name}${'低等高'[i]}于${i == 1 ? '均值' : '1个标准差'}时，${
                                i1.name
                            }对${d1.name}的${op.ifRes('orient', B)}向预测作用${op.ifRes(
                                'p*',
                                p,
                            )}，` +
                            ` B = ${B.toFixed(2)}, t(${df}) = ${t.toFixed(2)}, ${op.ifRes(
                                'p***',
                                p,
                            )}`,
                    )
    ...
    },
    {
        ...
        update() {
            return (
                'Johnson-Neyman' +
                this.dataElm(`m1_simple1`, 'pLine|正基准线,nLine|负基准线', {
                    label: m1.name,
                })
            );
        },
        output() {
            ...
            // pLine < nLine 图像斜率为正
            const desc =
                `${m1.name}${pLine < nLine ? '低' : '高'}于${pLine}个标准差时，${i1.name}对${
                    d1.name
                }的负向预测作用显著；` +
                `${m1.name}在${pLine}和${nLine}两个标准差之间时，${i1.name}对${d1.name}的预测作用不显著；` +
                `${m1.name}${pLine < nLine ? '高' : '低'}于${nLine}个标准差时，${i1.name}对${
                    d1.name
                }的正向预测作用显著`;
